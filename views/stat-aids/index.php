<?php $_GET['menu'] = 1;
$this->title = "ຕາຕາລາງບັນດາໂຄງການຊ່ວຍເຫຼືອຂອງຕ່າງປະເທດ ປະຈຳປີ";
  ...
  <div class="col-sm-12">
    <div class="panel panel-primary" style="margin-top: 2em" ng-show="year != null">
      ...
      <div class="panel-body {{mode=='input'?'':'hidden'}}">
        <div class="col-sm-4">
          <label for="">ຊື່ໂຄງການ</label>
          <input type="text" class="form-control" ng-model="model.name">
        </div>
        <div class="col-sm-2">
          <label for="">ເລກໝາຍສັນຍາ</label>
          <input type="text" class="form-control" ng-model="model.contract">
        </div>
        <div class="col-sm-2">
          <label for="">ເລີ່ມປະຕິບັດ</label>
          <input type="text" class="form-control" ng-model="model.start">
        </div>
        <div class="col-sm-2">
          <label for="">ສຳເລັດ</label>
          <input type="text" class="form-control" ng-model="model.finish">
        </div>
        <div class="col-sm-2">
          <label for="">ປະເພດໂຄງການ</label>
          <select class="form-control" ng-model="type" ng-options="g.name for g in types" ng-change="inquiry()">
          </select>
        </div>
        <div class="col-sm-4">
          <label for="">ມູນຄ່າຂອງໂຄງການ</label>
          <input type="text" class="form-control" ng-model="model.amount">
        </div>
        <div class="col-sm-8">
            ...
            <table class="table table-bordered table-hover" ng-show="models">
              <thead>
                <tr>
                  ...
                  <th class="text-center" rowspan="2">ລຶບ</th>
                </tr>
                <tr>
              ...
              <tbody>
                <tr ng-repeat="m in models" ng-click="remove(m)">
...
<script type="text/javascript">
  ...
  app.controller('statAidsController', function($scope, $http, $sce, $timeout) {
    $scope.url = 'index.php?r=stat-aids/';
    $scope.mode = 'read';
    ...
    $scope.inquiry = function() {
      if ($scope.year && $scope.model.province)
        $http.get($scope.url + 'inquiry&type=' + $scope.model.type.id + '&year=' + $scope.year.id)
        .then(function(r) {
          $scope.data.model = r;
        }, function(r) {
          $scope.response = r;
    ...
    $scope.save = function() {
      if ($scope.year && $scope.model) {
        $scope.model.type = $scope.type.id;
