 ...
 */
class StatOfficerContractController extends BaseController
{
    /**
    ...
    public function actionEnquiry($year)
    {
        ...
        $models = StatOfficerContractDetail::find()
            ->alias('d')
            ->select('d.*, g.name')
    ...
    public function actionInquiry($year, $group)
    {
        $user = Yii::$app->user->identity;
        $controller_id = Yii::$app->controller->id;
        $acton_id = Yii::$app->controller->action->id;
        ...
        $model = StatOfficerContractDetail::find()->alias('d')
            ->join('join', 'stat_officer_contract o', 'o.id = d.stat_officer_contract_id and o.phiscal_year_id=:year and d.ministry_group_id=:group', [
                ':year' => $year->id,
                ':group' => $group
            ])
            ->asArray()->one();
    ...
    public function actionSave($year)
    {
        ...
        try {
            $model = StatOfficerContract::find()->where([
            ...
            $detail = StatOfficerContractDetail::find()->alias('d')
                ->join('join', 'stat_officer_contract o', 'o.id = d.stat_officer_contract_id and o.phiscal_year_id=:year', [':year' => $year->id])
                ->where([
                    'ministry_group_id' => $post['Model']['group']
                ])
                ->one();
            ...
            if (!isset($detail)) {
                ...
                $detail->ministry_group_id = $post['Model']['group'];
            }
    ...
    public function actionUpload($year)
    {
        ...
        $menu = Menu::find()->where(['table_name' => 'stat_officer_contract'])->one();
        if (!isset($menu)) {
