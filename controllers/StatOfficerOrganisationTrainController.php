 ...
 */
class StatOfficerOrganisationTrainController extends BaseController
{
    public $columns = [
    ...
    public function actionUpload($year)
    {
        ...
        $menu = Menu::find()->where(['table_name' => 'stat_officer_organisation_train'])->one();
        if (!isset($menu)) {
    ...
    public function actionGetreferences($year)
    {
        ...
        $files = Attachment::find()->alias('a')
            ->join('join', 'menu m', 'm.id = a.menu_id and m.table_name=:table', [
                ':table' => 'stat_officer_organisation_train'
            ])
