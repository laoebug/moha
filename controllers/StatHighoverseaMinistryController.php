 ...
 */
class StatHighoverseaMinistryController extends BaseController
{
    public $columns = [
    ...
    public function actionGet()
    {
        ...
        $ministries = Ministry::find()
            ->where('deleted=0 and ministry_group_id in (1,2,4,5)')
    ...
    public function actionUpload($year)
    {
        ...
        $menu = Menu::find()->where(['table_name' => 'stat_highoversea_ministry'])->one();
        if (!isset($menu)) {
    ...
    public function actionGetreferences($year)
    {
        ...
        $files = Attachment::find()->alias('a')
            ->join('join', 'menu m', 'm.id = a.menu_id and m.table_name=:table', [
                ':table' => 'stat_highoversea_ministry'
            ])
