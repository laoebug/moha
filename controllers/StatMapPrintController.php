...
/**
 * StatMapPrintController implements the CRUD actions for StatMapPrint model.
 */
class StatMapPrintController extends BaseController
{
    public $tablename = 'stat_map_print';

    /**
     * Lists all StatMapPrint models.
     * @return mixed
     */
    ...
    public function actionEnquiry($year)
    {
        ...
        $models = StatMapPrint::find()->where(['phiscal_year_id' => $year->id])->asArray()->all();
        return json_encode([
            'models' => $models
        ]);
    }
    ...
    public function actionSave($year)
    {
        ...
        if (!isset($post['StatMapPrint'])) {
            MyHelper::response(HttpCode::BAD_REQUEST, Yii::t('app', 'Incorrect Request'));
            return;
        }
        try {
            $p = $post['StatMapPrint'];
            $model = isset($p['id']) ? StatMapPrint::find()
                ->where(['phiscal_year_id' => $year->id, 'id' => $p['id']])
                ->one() : new StatMapPrint();
            $model->load($post);
    ...
    public function actionPrint($year)
    {
        ...
        $models = StatMapPrint::find()->where(['phiscal_year_id' => $year->id])->all();
    ...
    public function actionDownload($year)
    {
        ...
        return $this->renderPartial('../ministry/excel', [
            'file' => 'Stat Map Print ' . $year->year . '.xls',
            'content' => $this->renderPartial('table', ['year' => $year, 'models' => $models])
        ]);
    }
