...
use app\models\StatHighcoinForeigner;
use app\models\StatHighcoinForeignerDetail;
...
/**
 * StatHighcoinForeignerController implements the CRUD actions for Stat3create model.
 */
class StatHighcoinForeignerController extends BaseController
{
    /**
     * Lists all StatHighcoinForeigner models.
     * @return mixed
     */
    ...
    public function actionGet()
    {
        ...
        $ministries = Ministry::find()->asArray()->all();
    ...
    public function actionEnquiry($year)
    {
        ...
        $model = StatHighcoinForeigner::find()->where(['phiscal_year_id' => $year->id])->one();
        if (!isset($model)) {
        ...
        $models = Ministry::find()->alias('m')
            ->select('m.*, d.*')
            ->join('left join', 'stat_highcoin_foreigner_detail d', 'd.ministry_id = m.id and d.stat_highcoin_foreigner_id=:id', [
                ':id' => $model->id
            ])
            ->asArray()->all();
    ...
    public function actionInquiry($year, $province)
    {
        ...
        $model = StatHighcoinForeignerDetail::find()->alias('d')
            ->join('join', 'stat_highcoin_foreigner l', 'l.id = d.stat_highcoin_foreigner_id and l.phiscal_year_id=:year', [':year' => $year->id])
            ->where(['ministry_id' => $province])
            ->asArray()->one();
    ...
    public function actionSave($year)
    {
        ...
        try {
            $model = StatHighcoinForeigner::find()
                ->where(['phiscal_year_id' => $year->id])
                ->one();
            if (!isset($model)) {
                $model = new StatHighcoinForeigner();
            ...
            $detail = StatHighcoinForeignerDetail::find()
                ->where(['stat_3create_id' => $model->id, 'province_id' => $post['Stat3createDetail']['province']['id']])
                ->one();
            if (!isset($detail)) {
                $detail = new StatHighcoinForeignerDetail();
                $detail->ministry_id = $post['model']['ministry']['id'];
                $detail->stat_highcoin_foreigner_id = $model->id;
            }
            $detail->ministry_id = @$post['model']['ministry']['id'];
            $detail->remark = @$post['model']['remark'];
            $detail->award_id = @$post['model']['award']['id'];
            $detail->national_group = @$post['model']['national_group'];
            $detail->national_personal = @$post['model']['national_personal'];
            $detail->million_group = @$post['model']['million_group'];
            $detail->million_personal = @$post['model']['million_personal'];
            $detail->hornor_group = @$post['model']['hornor_group'];
            $detail->hornor_personal = @$post['model']['hornor_personal'];
    ...
    public function actionDownload($year)
    {
        ...
        return $this->renderPartial('../ministry/excel', [
            'file' => 'Stat High Coin Honor ' . $year->year . '.xls',
            'content' => $this->renderPartial('table', ['year' => $year, 'models' => $models])
        ]);
    }
