 ...
 */
class StatOfficerSalaryController extends BaseController
{
    /**
    ...
    public function actionEnquiry($year)
    {
        ...
        $steps = SalaryLevel::find()->where(['deleted' => 0])->orderBy('position')->all();
        $models = [];
        foreach ($steps as $step) {
            $m = StatOfficerSalaryDetail::find()->alias('d')
                ->select('d.*')->addSelect(['name' => 'l.name', 'step' => 'r.name'])
                ...
                ->join('join', 'salary_level r', 'r.id=d.salary_level_id')
                ->where(['salary_level_id' => $step->id])
                ->asArray()->all();
            if (count($m) > 0)
                $models[$step->id] = $m;
        }
    ...
    public function actionInquiry($year, $level, $step)
    {
        $user = Yii::$app->user->identity;
        $controller_id = Yii::$app->controller->id;
        $acton_id = Yii::$app->controller->action->id;
    ...
    public function actionSave($year)
    {
        ...
        if (isset($post)) {
            ...
            try {
                ...
                $detail->salary_level_id = $post['Model']['step']['id'];
    ...
    public function actionDownload($year)
    {
        ...
        return $this->renderPartial('../ministry/excel', [
            'file' => 'Stat Officer Salary ' . $year->year . '.xls',
            'content' => $this->renderPartial('table', [
                'models' => $models,
                'year' => $year
            ])
    ...
    public function actionUpload($year)
    {
        ...
        $menu = Menu::find()->where(['table_name' => 'stat_officer_salary'])->one();
        if (!isset($menu)) {
